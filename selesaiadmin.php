...
function acakangkahuruf($panjang)
{
	...
	if(isset($_POST['finish'])) {
		...
		echo "<p>Terimakasih telah berbelanja di toko online kami, <b>" . $data['transaksi_nama'] . "</b>";
			if (isset($data)) {
				$sql_detail = "SELECT * FROM data_obat,transaksi,pesanan WHERE pesanan.barang_id = data_obat.id_obat AND transaksi.transaksi_id = '" . $data['transaksi_id'] . "' AND pesanan.transaksi_id = '" . $data['transaksi_id'] . "' AND transaksi.transaksi_id = pesanan.transaksi_id";
				$query_detail = mysql_query($sql_detail);
				echo "<p>Rincian Obat yang anda beli : ";
				$i = 1;
				while($data_detail = mysql_fetch_array($query_detail) or mysql_error()) {
					echo "<p>$i. </td><td>$data_detail[nama_obat], </td><td>Harga satuan : $data_detail[harga_obat], </td><td>Jumlah : $data_detail[transaksi_jumlah]</td></tr>".mysql_error();
					$i++;
				}
				$ongkir = 21000;
